...
export default function EditProduct({ params }: Props) {
    ...
    const fetchInitialData = async () => {
      try {
        ...
        if (response.ok) {
          ...
          // Next fetch product
          const prodRes = await fetch("/api/v1/products/" + slug);
          if (prodRes.ok) {
             ...
             setGenderTag(p.tags?.[0] || "unisex");
             if (p.imageUrls) setPreviewUrls(p.imageUrls);
             
             if (p.category) {
               const catMatch = data.data.find((c: any) => c.name === p.category);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!productRes.ok) throw new Error(productData.message || "Failed to update product");
  ...
  return (
    <div className="space-y-6 max-w-4xl mx-auto pb-12">
      ...
      <div className="flex items-center space-x-4">
        ...
        <div>
          <h1 className="text-2xl font-bold text-asc-matte">Edit Product</h1>
          <p className="mt-1 text-sm text-asc-charcoal">
            Update existing apparel item details
          </p>
        </div>
      </div>
